...
res = np.array([100]) # Resolutions tested at (number of grid spacing between x0 and x1)               0.2/100 ~ 1000x500 res
...
for j in xrange(len(res)):
    ...
    if DP45 == True:
        ...
        # marks = ['x','o',None]
        # ls = [':',':','-']
        plt.plot(np.linspace(x0,x1,res[j]),ftle[0],':', alpha=dp_line_alpha, marker='x', ms = 3)
